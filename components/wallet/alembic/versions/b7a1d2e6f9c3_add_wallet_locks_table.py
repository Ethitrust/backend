...
def upgrade() -> None:
    ...
    if not inspector.has_table("wallet_locks"):
        op.create_table(
            "wallet_locks",
            sa.Column(
    ...
    existing_indexes = {idx["name"] for idx in inspector.get_indexes("wallet_locks")}

    if "ix_wallet_locks_wallet_id" not in existing_indexes:
        op.create_index("ix_wallet_locks_wallet_id", "wallet_locks", ["wallet_id"])
    if "ix_wallet_locks_source_id" not in existing_indexes:
        op.create_index("ix_wallet_locks_source_id", "wallet_locks", ["source_id"])
    if "ix_wallet_locks_status" not in existing_indexes:
        op.create_index("ix_wallet_locks_status", "wallet_locks", ["status"])
    if "ix_wallet_locks_source_type_source_id" not in existing_indexes:
        op.create_index(
